 ...
 *
 * 缺点：
 * 1、 违背单一职责：单点故障危害，由于工厂类集中了所有产品创建逻辑，一旦不能正常工作，整个系统都要受到影响，违背单一职责
 * 2、 违背开闭原则：系统扩展性不好，一旦添加新产品就不得不修改工厂逻辑，在产品类型较多时，有可能造成工厂逻辑过于复杂，不利于系统的扩展和维护，违背了开闭原则
 *
 * 使用场景：
 * 工厂类负责创建的产品种类较少、变动不大, 但当产品种类增加时，可能会导致工厂类膨胀，维护变得困难
 */
public class SimplePizzaFactory {

    /**
     * 客户端可以通过工厂方法和对应的产品参数创建不同的产品对象
     */
